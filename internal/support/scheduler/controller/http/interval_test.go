...
import (
	...
	"github.com/agile-edge/edgex/internal/support/scheduler/container"
	dbMock "github.com/agile-edge/edgex/internal/support/scheduler/infrastructure/interfaces/mocks"
...
func TestIntervalByName(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			e := echo.New()
			reqPath := fmt.Sprintf("%s/%s", common.ApiIntervalByNameEchoRoute, testCase.intervalName)
			...
			c.SetParamValues(testCase.intervalName)
			err = controller.IntervalByName(c)
			require.NoError(t, err)
...
func TestDeleteIntervalByName(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			c.SetParamNames(common.Name)
			c.SetParamValues(testCase.intervalName)
			err = controller.DeleteIntervalByName(c)
			require.NoError(t, err)
			var res commonDTO.BaseResponse
			err = json.Unmarshal(recorder.Body.Bytes(), &res)
			require.NoError(t, err)
...
func TestPatchInterval(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			c := e.NewContext(req, recorder)
			err = controller.PatchInterval(c)
			require.NoError(t, err)
